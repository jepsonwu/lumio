...
class GenerateProviderCommand extends GeneratorCommand
{
    ...
    {
        parent::fire();
        $this->generateLumenProviders();
    }


    protected function generateLumenProviders()
    {
        $defaultProviders = ['App', 'Auth', 'Event'];

        $module = $this->laravel['modules']->findOrFail($this->getModuleName());

        array_map(function($provider) use ($module) {

            $stubPath = '/lumen/' . $provider . 'ServiceProvider.stub';

            $path = $this->getDestinationFilePathUsing($provider);

            $contents = (new Stub($stubPath, ['NAMESPACE' => $this->getClassNamespace($module)]))->render();
    ...
    protected function getDestinationFilePathUsing($provider)
    {
        $path = $this->laravel['modules']->getModulePath($this->getModuleName());

        $generatorPath = $this->laravel['modules']->config('paths.generator.provider');

        return $path . $generatorPath . '/' . $provider . 'ServiceProvider.php';
    }

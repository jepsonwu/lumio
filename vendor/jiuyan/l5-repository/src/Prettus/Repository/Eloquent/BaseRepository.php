...
abstract class BaseRepository implements RepositoryInterface, RepositoryCriteriaInterface
{
     ...
     * @var Collection
     */
    protected $inTransactionRepository;
    ...
    public function increment($id, $column, $amount, $extra = [])
    {
        $this->applyScope();
        $temporarySkipPresenter = $this->skipPresenter;
        $this->skipPresenter(true);

        if (!$this->model->find($id)->increment($column, $amount, $extra)) {
            return false;
        }
    ...
    public function decrement($id, $column, $amount, $extra = [])
    {
        $this->applyScope();
        $temporarySkipPresenter = $this->skipPresenter;
        $this->skipPresenter(true);

        if (!$this->model->find($id)->decrement($column, $amount, $extra)) {
            return false;
        }
        $model = $this->model->find($id);
        $this->skipPresenter($temporarySkipPresenter);
    ...
    protected function skipCacheForTransaction(Collection $inTransactionRepository, $status = true)
    {
        $inTransactionRepository->each(function (BaseRepository $baseRepository) use ($status) {
            $baseRepository->enableCache() && $baseRepository->skipCache($status);
        });
    }
    ...
    public function beginTransaction(Collection $inTransactionRepository)
    {
        ...
        $this->inTransactionRepository = $inTransactionRepository;
        $this->inTransactionRepository && $this->skipCacheForTransaction($this->inTransactionRepository);
    }

    public function rollBack()
    {
        $this->model->rollBack();
        $this->inTransactionRepository && $this->skipCacheForTransaction($this->inTransactionRepository, false);
    }
    ...
    public function commit()
    {
        ...
        if ($this->inTransactionRepository) {
            $this->inTransactionRepository->each(function (BaseRepository $baseRepository) {
                event(new RepositoryEntityCreated($baseRepository, $this->model));
                event(new RepositoryEntityUpdated($baseRepository, $this->model));
                event(new RepositoryEntityDeleted($baseRepository, $this->model));
            });

            $this->skipCacheForTransaction($this->inTransactionRepository, false);
        }
    }
}

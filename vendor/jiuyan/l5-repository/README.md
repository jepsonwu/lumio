...
- beginTransaction(Collection $inTransactionRepository)
- rollBack()
- commit()
...
class PostRepository extends BaseRepository {
    ...
    /**
     * Set slice field's value to ensure table's name before executing all、find functions
     */
     public function setSliceTableInfo($sliceFieldVal,$tableCount=self::TABLECOUNT){
        $this->model->setSliceTableInfo($sliceFieldVal, $tableCount);
     }
}
```
...
##### Youn also can set module path(--module=XXX) to replcae 'basePath' by command
...
```terminal
php artisan make:entity Post --module=XXX
```
...
```terminal
php artisan make:repository Post --module=XXX
```

To generate a repository for your Post model with Blog namespace, use the following command

```terminal
php artisan make:repository "Blog\Post"  --module=XXX
```

Added fields that are fillable

```terminal
php artisan make:repository "Blog\Post" --fillable="title,content"  --module=XXX
```

To add validations rules directly with your command you need to pass the `--rules` option and create migrations as well:

```terminal
php artisan make:entity Cat --fillable="title:string,content:text" --rules="title=>required|min:2, content=>sometimes|min:10"  --module=XXX
```
...
```php
$this->Repository->beginTransaction(new Collection(
    [
        $this->Repository,
        $this->RepositoryBack
    ])
...
```terminal
php artisan make:criteria My  --module=XXX
```
...
```terminal
php artisan make:transformer Post --module=XXX
```
...
```terminal
php artisan make:presenter Post --module=XXX
```

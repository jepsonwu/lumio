...
class Client{
	 ...
	 * 
	 */
	function __construct($url,$userName,$userPass,$timeout = 0, $response_timeout = 30)
	{
	...
	*/
     ...
     * @param $pwd
     */
    function batchGetUpSms($name, $pwd) {
        $param = array(
            'userId' => $name,
            'password' => $pwd,
            'iReqType' => 1,
        ...
        $result = array();
        $response = $this->soap->call('MongateGetDeliver', $param, $this->namespace);
        if(isset($response['MongateGetDeliverResult']['string'])) {
            if(is_string($response['MongateGetDeliverResult']['string'])) {
                $result[] = $this->makeResultItem($response['MongateGetDeliverResult']['string']);
            }
            if(is_array($response['MongateGetDeliverResult']['string'])) {
                foreach($response['MongateGetDeliverResult']['string'] as $string) {
                    $result[] = $this->makeResultItem($string);
                }
            }
        }
        return $result;
    ...
    private function makeResultItem($string) {
        $arr = explode(',', $string);
        if(is_array($arr)) {
            $arr = array_map('trim', $arr);
            $time = $arr[1] ? strtotime($arr[1]) : '';
            $contentArr = array_slice($arr, 6);
